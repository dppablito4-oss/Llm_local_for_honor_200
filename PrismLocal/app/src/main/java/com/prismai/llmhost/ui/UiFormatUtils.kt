    ...
    return compact
        ...
        .joinToString(" ") { token ->
            when {
                ...
                token.startsWith("qwen", ignoreCase = true) ->
                    "Qwen${token.drop(4)}"
                token.startsWith("smollm", ignoreCase = true) ->
                    "SmolLM${token.drop(6)}"
                token.equals("instruct", ignoreCase = true) -> "Instruct"
                token.matches(Regex("\\d+(?:\\.\\d+)?[a-zA-Z]?")) -> token.uppercase(Locale.US)
