...
class ModelStorageManager(private val context: Context) {
    ...
    fun importModelFromStream(
        ...
        return try {
            ...
            pruneInactiveVersionEntries(manifest, versionId)
    ...
    private fun pruneInactiveVersionEntries(manifest: JSONObject, activeVersionId: String) {
        val versions = manifest.optJSONObject("versions") ?: return
        val inactiveVersions = mutableListOf<String>()
        val keys = versions.keys()
        while (keys.hasNext()) {
            val versionId = keys.next()
            if (versionId != activeVersionId) {
                inactiveVersions += versionId
            }
        }
        inactiveVersions.forEach { versions.remove(it) }
    }
