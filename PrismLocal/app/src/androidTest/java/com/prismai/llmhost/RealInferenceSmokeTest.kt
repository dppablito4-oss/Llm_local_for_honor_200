...
class RealInferenceSmokeTest {
    ...
    fun realTinyModelOneTokenSmoke() = runBlocking {
        ...
        try {
            ...
            assertTrue(
                ...
                chunks.any { it.isTerminal && it.terminalReason in setOf("EOF", "MAX_TOKENS") },
            )
        } finally {
    ...
    private fun requireTinyModel(): ModelStorageManager.ActiveModelInfo {
        ...
        assertNotNull(
            "Missing real $TEST_MODEL_ID GGUF manifest/model under app private files",
            model
        )
    ...
    private fun stageTinyModel(instrumentation: android.app.Instrumentation) {
        ...
        val modelsDir = File(targetContext.filesDir, "models")
