...
struct Engine::Impl {
    ...
    void runRealGeneration(const std::shared_ptr<GenerationSession>& session, const std::string& prompt) {
        ...
        if (session->config.continue_from_context) {
            ...
            if (!shiftRuntimeContextIfNeeded(*runtime, session->config.max_tokens, false,
                                             cache_identity)) {
            ...
            if (!shiftRuntimeContextIfNeeded(*runtime, tokens_to_decode + session->config.max_tokens, true,
                                             cache_identity)) {
                ctrl->error_code.store(426, std::memory_order_release);
...
void Engine::cancelGeneration(int generation_id) {
    ...
    // If no newer generation claimed the ring while the cancelled worker joined,
    // discard any leftover tokens from the cancelled stream. Keep the terminal
    // state intact so callers waiting for CANCELLED can still observe it.
    {
        std::lock_guard<std::mutex> lock(impl_->mu);
        auto* ctrl = impl_->buffers.control;
        const bool same_generation =
            ctrl->generation_id.load(std::memory_order_acquire) == static_cast<uint32_t>(generation_id);
        const bool active_reused_id =
            impl_->active_session &&
            impl_->active_session->generation_id == static_cast<uint32_t>(generation_id);
        if (same_generation && !active_reused_id) {
            clearRing(ctrl);
        }
    }
}
